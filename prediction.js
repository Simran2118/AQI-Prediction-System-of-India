...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const stateCityMap = JSON.parse(stateDataElement.textContent);

    console.log("STATE MAP:", stateCityMap);
    ...
    stateDropdown.addEventListener("change", function () {
        ...
        const cleanedSelected = selectedState.trim();

        cityDropdown.innerHTML = '<option value="">--Select City--</option>';

        let found = false;

        Object.keys(stateCityMap).forEach(function (key) {

            if (key.trim() === cleanedSelected) {

                stateCityMap[key].forEach(function (city) {
    
                    if (!city || city === "NaN") return;
        ...
        if (!found) {
            console.warn("No cities found for:", cleanedSelected);
...
function displayResults(data) {
    ...
    document.getElementById('summSo2').textContent = (data.input_data.so2 ?? 0) + ' μg/m³';
    document.getElementById('summNo2').textContent = (data.input_data.no2 ?? 0) + ' μg/m³';
    document.getElementById('summRspm').textContent = (data.input_data.rspm ?? 0) + ' μg/m³';
    document.getElementById('summSpm').textContent = (data.input_data.spm ?? 0) + ' μg/m³';

    const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    document.getElementById('summDate').textContent = monthNames[data.input_data.month - 1] + ' ' + data.input_data.year;
    document.getElementById('summLocation').textContent =
        (data.input_data.city || 'N/A') + ', ' + (data.input_data.state || 'N/A');
...
document.getElementById("liveDataButton").addEventListener("click", function() {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            showNotification('Live prediction completed!', 'success');
