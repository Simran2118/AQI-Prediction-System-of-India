...
API_KEY = "dee980aafb6ec0f95ac988c9ec567b52"
...
try:
    ...
    feature_info = {
        'feature_columns': ['so2', 'no2', 'rspm', 'spm', 'year', 'month', 'state_encoded', 'type_encoded' ,'city_encoded'],
        'target_column': 'pm2_5'
    }
...
try:
    ...
    le_city, le_state, le_type = None, None, None
...
def prediction():
    if df is None:
        return render_template('prediction.html', states=[], state_city_map={})

    df_copy = df.copy()

    # Clean columns
    df_copy['state'] = df_copy['state'].astype(str).str.strip()

    
    city_col = 'location' if 'location' in df_copy.columns else 'city'

    df_copy[city_col] = df_copy[city_col].astype(str).str.strip()

    # Remove invalid values
    df_copy = df_copy[df_copy[city_col].notna()]
    df_copy = df_copy[df_copy[city_col] != 'NaN']

    # Create mapping
    state_city_map = (
        df_copy.groupby('state')[city_col]
        .unique()
        .apply(list)
        .to_dict()
    )

    states = sorted(state_city_map.keys())
...
def get_live_data(city):
    ...
    return {
        ...
        "rspm": components.get("pm10", 0),
        "spm": components.get("pm2_5", 0)
    }
    ...
    try:            
        ...
        if data.get("live") == True:
            ...
            if live_data:
                ...
                return jsonify({
                    'success': False,
                    'error': 'Live data not available for this city'
                }), 400
        else :
        ...
        if feature_info and 'feature_columns' in feature_info:
            ...
            if 'state_encoded' in feature_info['feature_columns']:
                if le_state:
                    try:
                        input_data['state_encoded'] = [le_state.transform([state])[0]]
                    except:
            ...
            if 'type_encoded' in feature_info['feature_columns']:
                if le_type:
                    try:
                        input_data['type_encoded'] = [le_type.transform([area_type])[0]]
                    except:
            ...
            # City encoding
            if 'city_encoded' in feature_info['feature_columns']:
                if le_city:
                    try:
                        input_data['city_encoded'] = [le_city.transform([city])[0]]
                    except:
